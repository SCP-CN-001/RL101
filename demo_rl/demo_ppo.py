...
def train(
    env: gym.Env, agent: PPO, action_processor,
    ...
    for i in range(n_step):
        ...
        if i % 1000 == 0:
            if len(scores) < 100:
                ...
                average_return = np.mean(scores[-100:])
...
def tensorboard_writer(env_name):
    ...
    writer_path = "./logs/ppo/%s/%s/" % (env_name, timestamp)
...
def PPO_mujoco(env_name):
    ...
    # Generate agent
    agent = PPO(configs)
    action_processor = MujocoActionProcessor(env.action_space)
    # Generate tensorboard writer
    writer = tensorboard_writer(env_name)
    train(env, agent, action_processor, n_step, env_name, writer)
